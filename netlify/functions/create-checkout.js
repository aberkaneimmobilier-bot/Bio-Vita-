  ...
  try {
    ...
    for (const item of cart) {
      ...
      if (!Number.isInteger(qty) || qty < 1 || qty > 999) {
        return { statusCode: 400, body: JSON.stringify({ error: "Quantite invalide." }) };
      }
      line_items.push({
        price_data: {
          currency: "cad",
    ...
    // Crée un client Stripe pré-rempli avec les coordonnées du site
    let customerId;
    if (customer.email) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items,
      customer: customerId,                  // pré-remplit email + adresse
      // On NE demande PAS l'adresse de livraison à nouveau (déjà saisie sur le site)
